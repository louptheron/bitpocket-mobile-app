...
@Injectable()
export class EthereumAverageExchangeService implements CurrencyExchangeService {
    ...
    getExchangeRates() : Promise<Array<CurrencyExchangeRate>> {
        return new Promise((resolve, reject) => {
            try {
                this.http.get('https://apiv2.bitcoinaverage.com/indices/global/ticker/short?crypto=ETH')
                    .subscribe(
                        response => {
    ...
    getExchangeRate(code:string) : Promise<CurrencyExchangeRate> {
        return new Promise((resolve, reject) => {
            try {
                this.http.get('https://apiv2.bitcoinaverage.com/indices/global/ticker/short?crypto=ETH&fiats=' + code)
                    .subscribe(
                        response => {                            
                            resolve(this.prepareOne(code, response['ETH' + code]));
                        } ,
                        error => {
                            reject();
                        }
